...
test("avatar skin copy matches the playful unlock list", () => {
  ...
  assert.equal(PLAYER_AVATAR_SKIN_UNLOCKS.basketball.label, "传奇王者 100 星解锁");
...
test("special avatar skins unlock from donation, king rank, legend stars, and luck", () => {
  ...
  assert.equal(getPlayerAvatarSkinUnlockState("basketball", fullStarsProgress).unlocked, false);
  assert.equal(getPlayerAvatarSkinUnlockState("basketball", markLegend100SkinUnlocked(fullStarsProgress)).unlocked, true);
...
test("avatar skin display keeps unlocked skins first and preserves the configured unlock order", () => {
  ...
  assert.deepEqual(
    ...
    ["cyan", "signal", "target", "mint", "slate", "sand", "pine", "ivory", "blade", "paw", "pig", "basketball", "arcade"],
  );
  assert.deepEqual(
    getPlayerAvatarSkinDisplayItems(searchClearedProgress).map((item) => item.skin),
    ["cyan", "mint", "pig", "signal", "target", "slate", "sand", "pine", "ivory", "blade", "paw", "basketball", "arcade"],
  );
});

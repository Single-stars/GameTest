...
export function ResultScreen({
  ...
  const [avatarEntryAnimating, setAvatarEntryAnimating] = useState(false);
  ...
  const openAvatarLab = useCallback(() => {
    if (avatarEntryAnimating) return;
    clearAvatarEntryTimer();
    setAvatarEntryAnimating(true);
    avatarEntryTimerRef.current = window.setTimeout(() => {
      avatarEntryTimerRef.current = null;
      setAvatarEntryAnimating(false);
      onOpenAvatarLab();
    }, AVATAR_LAB_ENTRY_ANIMATION_MS);
  }, [avatarEntryAnimating, clearAvatarEntryTimer, onOpenAvatarLab]);
  ...
  return (
    <section className="result-screen">
      <div className="result-card rank-card">
        ...
        <button
          aria-label="进入皮肤测试"
          className={`rank-avatar-entry ${avatarEntryAnimating ? "playing" : ""}`}
          type="button"
          onPointerDown={openAvatarLab}
        >
          <PlayerAvatar
            action={avatarEntryAnimating ? "celebrate" : "idle"}
            effect={avatarEntryAnimating ? "sparkles" : "none"}
            expression={avatarEntryAnimating ? "happy" : "neutral"}
            skin={avatarSkin}
            size={46}
            visualScale={1.02}
          />
      ...
      <div className="radar-card-shell">
        <div className="radar-actions" aria-label="结果操作">
          <button
            aria-label="生成分享图片"

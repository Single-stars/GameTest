...
test("app page delegates round rendering and remaining screen shells to feature modules", () => {
  ...
  assert.doesNotMatch(appPageSource, /getInitialStageFromLocation/);
  assert.doesNotMatch(appPageSource, /useState<Stage>\(\(\) =>/);
  ...
  assert.match(appPageSource, /transitionToStage\(trialsRef\.current\.length > 0 \? "result" : "home"\)/);
  ...
  assert.match(appPageSource, /const openOutdoorAdventure = useCallback[\s\S]*setHomeworldReturnPose\(homeworldPlayerPoseRef\.current\)/);
  assert.match(appPageSource, /const openHomeworldPortalRoom = useCallback[\s\S]*setHomeworldReturnPose\(homeworldPlayerPoseRef\.current\)/);
  assert.match(appPageSource, /const joinHomeworldPortalRoom = useCallback[\s\S]*setHomeworldReturnPose\(homeworldPlayerPoseRef\.current\)/);
  assert.match(appPageSource, /transitionToRoute\("\/multiplayer\?homeworld=1&host=1"/);
  assert.match(appPageSource, /transitionToRoute\(`\/multiplayer\?homeworld=1&room=\$\{encodeURIComponent\(roomCode\)\}`/);
...
test("base play frame has a dismissible full-screen tutorial overlay", () => {
  ...
  assert.match(playFrameCss, /\.base-tutorial-panel\s*\{[\s\S]*max-width:\s*520px;/);

...
test("flappy endless gravity changes use sine portals, early triggers, and smooth gravity blending", () => {
  ...
  assert.match(gravityChangeSource, /gravityFlipTransitionRef\.current = \{/);
  assert.match(gravityChangeSource, /playerY: current\.playerY/);
  assert.match(gravityChangeSource, /current\.started = false;/);
  assert.match(gravityChangeSource, /current\.playerVy = 0;/);
  ...
  assert.doesNotMatch(gravityChangeSource, /gravityFlippedRef\.current = targetFlipped;/);
  assert.doesNotMatch(gravityChangeSource, /current\.invincibleUntil = Math\.max\(current\.invincibleUntil, nextTime \+ FLAPPY_GRAVITY_CHANGE_INVINCIBLE_SECONDS\);/);
  assert.match(flappyRuntimeSource, /type FlappyGravityFlipTransition = \{/);
  assert.match(flappyRuntimeSource, /const gravityFlipTransitionRef = useRef<FlappyGravityFlipTransition \| null>\(null\);/);
  assert.match(flappyRuntimeSource, /const \[gravityFlipAwaitingJump, setGravityFlipAwaitingJump\] = useState\(false\);/);
  assert.match(gravityChangeSource, /setGravityFlipAwaitingJump\(true\);/);
  assert.match(flappyRuntimeSource, /if \(gravityFlipTransitionRef\.current\) return;/);
  assert.match(flappyRuntimeSource, /setGravityFlipAwaitingJump\(false\);/);
  assert.match(flappyRuntimeSource, /const activeGravityFlipTransition = gravityFlipTransitionRef\.current;/);
  assert.match(flappyRuntimeSource, /gravityFlippedRef\.current = activeGravityFlipTransition\.targetFlipped;/);
  ...
  assert.match(flappyRuntimeSource, /view\.started \|\| gravityFlipAwaitingJump \? "started" : ""/);
  ...
  assert.match(cssSource, /\.flappy-stage\.gravity-flip-preparing \.flappy-world/);
  assert.match(cssSource, /\.flappy-stage\.gravity-flip-preparing \.flappy-world\s*{[\s\S]*scale\(0\.965\)/);

...
test("flappy endless gravity changes use sine portals, early triggers, and smooth gravity blending", () => {
  ...
  assert.doesNotMatch(flappyRuntimeSource, /flappy-anomaly-sine-line line-/);
  assert.match(anomalyMarkerCss, /\.flappy-anomaly-marker/);
  assert.match(anomalyMarkerCss, /\.flappy-anomaly-sine-line/);
  assert.doesNotMatch(anomalyMarkerCss, /\.flappy-anomaly-sine-line\.line-/);
  ...
  assert.doesNotMatch(gravityChangeSource, /gravityFlipTransitionRef\.current = \{/);
  assert.doesNotMatch(gravityChangeSource, /playerY: current\.playerY/);
  assert.doesNotMatch(gravityChangeSource, /current\.started = false;/);
  assert.doesNotMatch(gravityChangeSource, /current\.playerVy = 0;/);
  assert.doesNotMatch(gravityChangeSource, /setGravityFlipFeedbackActive/);
  assert.match(gravityChangeSource, /setGravityFlipTransitionActive\(true\);/);
  ...
  assert.doesNotMatch(flappyRuntimeSource, /type FlappyGravityFlipTransition = \{/);
  assert.doesNotMatch(flappyRuntimeSource, /const gravityFlipTransitionRef = useRef/);
  assert.doesNotMatch(flappyRuntimeSource, /const \[gravityFlipAwaitingJump, setGravityFlipAwaitingJump\] = useState\(false\);/);
  assert.doesNotMatch(gravityChangeSource, /setGravityFlipAwaitingJump\(true\);/);
  assert.doesNotMatch(flappyRuntimeSource, /if \(gravityFlipTransitionRef\.current\) return;/);
  assert.doesNotMatch(flappyRuntimeSource, /setGravityFlipAwaitingJump\(false\);/);
  assert.doesNotMatch(flappyRuntimeSource, /const activeGravityFlipTransition = gravityFlipTransitionRef\.current;/);
  assert.doesNotMatch(flappyRuntimeSource, /gravityFlippedRef\.current = activeGravityFlipTransition\.targetFlipped;/);
  assert.match(flappyRuntimeSource, /current\.invincibleUntil = Math\.max\(current\.invincibleUntil, nextTime \+ FLAPPY_GRAVITY_CHANGE_INVINCIBLE_SECONDS\);/);
  ...
  assert.doesNotMatch(flappyRuntimeSource, /gravityFlipFeedbackActive \? "gravity-flip-feedback" : ""/);
  assert.match(flappyRuntimeSource, /gravityFlipTransitionActive \? "gravity-flip-preparing" : ""/);
  assert.doesNotMatch(flappyRuntimeSource, /gravityFlipAwaitingJump/);
  assert.match(flappyRuntimeSource, /view\.started \? "started" : ""/);
  assert.doesNotMatch(cssSource, /\.flappy-stage\.gravity-flip-feedback::after/);
  assert.match(cssSource, /\.flappy-world\s*{[\s\S]*transition:\s*transform 280ms cubic-bezier\(0\.16,\s*1,\s*0\.3,\s*1\);/);
  assert.doesNotMatch(cssSource, /\.flappy-stage\.gravity-flip-preparing \.flappy-world/);
  assert.doesNotMatch(cssSource, /\.flappy-stage\.gravity-flip-preparing \.flappy-world\s*{[\s\S]*scale\(0\.965\)/);
  assert.match(cssSource, /\.flappy-stage\.gravity-flip-preparing\s*{[\s\S]*--difficulty-wave-opacity:/);
  assert.doesNotMatch(cssSource, /scale\(1\.035\)/);
...
test("flappy super dash uses one soft velocity-aligned tail and clamps the player inside the stage", () => {
  ...
  const cssSource = readAppCssSource();
  const flappyRuntimeSource = componentSource.slice(componentSource.indexOf("export function FlappyPrototype"));
  const trailCss = cssSource.slice(
    cssSource.indexOf(".flappy-speed-trail-flow"),
    cssSource.indexOf(".flappy-player", cssSource.indexOf(".flappy-speed-trail-flow")),
  );

  assert.match(componentSource, /const FLAPPY_SPEED_TRAIL_LENGTH = 118;/);
  assert.match(componentSource, /playerVy: frame\.playerVy,/);
  assert.match(componentSource, /const showFlappySpeedTrail = avatarEffect === "wind" && view\.status === "playing" && view\.started;/);
  assert.match(componentSource, /const speedTrailScreenVx = \(activeViewReverseDirection \? -1 : 1\) \* numberParam\(viewFlappyParams, "speed", speed\) \* FLAPPY_SUPER_DASH_SPEED_MULTIPLIER;/);
  assert.match(componentSource, /const flappySpeedTrailRef = useRef<HTMLSpanElement \| null>\(null\);/);
  assert.match(componentSource, /const flappySpeedTrailVisualRef = useRef<FlappySpeedTrailVisual \| null>\(null\);/);
  assert.match(componentSource, /function smoothFlappySpeedTrailVisual\(/);
  assert.match(componentSource, /const speedTrailAngleDeg = Math\.atan2\(current\.playerVy, speedTrailScreenVx\) \* \(180 \/ Math\.PI\);/);
  assert.match(componentSource, /const smoothedTrail = smoothFlappySpeedTrailVisual\(flappySpeedTrailVisualRef\.current,/);
  assert.match(componentSource, /flappySpeedTrailVisualRef\.current = smoothedTrail;/);
  assert.match(componentSource, /flappySpeedTrailNode\.style\.setProperty\("--flappy-speed-trail-angle", `\$\{smoothedTrail\.angleDeg\}deg`\);/);
  assert.match(componentSource, /const speedTrailStyle = \{[\s\S]*"--flappy-speed-trail-length": `\$\{FLAPPY_SPEED_TRAIL_LENGTH\}px`/);
  assert.match(componentSource, /const attachFlappySpeedTrail = useCallback\(/);
  assert.match(componentSource, /flappySpeedTrailRef\.current = node;/);
  assert.match(componentSource, /node\.style\.setProperty\("--flappy-speed-trail-angle", `\$\{initialAngleDeg\}deg`\);/);
  assert.match(componentSource, /node\.style\.setProperty\("--flappy-speed-trail-x", `\$\{playerScreenX\}px`\);/);
  assert.match(componentSource, /node\.style\.setProperty\("--flappy-speed-trail-y", `\$\{view\.playerY\}px`\);/);
  assert.match(componentSource, /if \(!showFlappySpeedTrail\) flappySpeedTrailVisualRef\.current = null;/);
  assert.match(componentSource, /className="flappy-speed-trail-flow" ref=\{attachFlappySpeedTrail\}/);
  assert.doesNotMatch(componentSource, /renderedSpeedTrailVisual/);
  assert.doesNotMatch(componentSource, /FLAPPY_SPEED_TRAIL_PIECES|flappy-speed-trail-point|type FlappyTrailPoint|trailPointsRef|setTrailPoints|recordFlappySpeedTrailPoint|activeViewCameraX/);
  assert.match(componentSource, /function clampFlappySuperDashY\(y: number, vy: number, stageHeight: number\)/);
  assert.match(flappyRuntimeSource, /const unclampedNextY = current\.playerY \+ nextVy \* delta;/);
  assert.match(flappyRuntimeSource, /const dashClamp = superDashActive \? clampFlappySuperDashY\(unclampedNextY, nextVy, stageHeight\) : null;/);
  assert.match(flappyRuntimeSource, /const nextY = dashClamp\?\.y \?\? unclampedNextY;/);
  assert.match(flappyRuntimeSource, /current\.playerVy = dashClamp\?\.vy \?\? nextVy;/);
  assert.match(cssSource, /\.flappy-speed-trail\s*{[\s\S]*z-index:\s*8;/);
  assert.match(trailCss, /width:\s*var\(--flappy-speed-trail-length,\s*118px\);/);
  assert.match(trailCss, /height:\s*62px;/);
  assert.match(trailCss, /border-radius:\s*61% 39% 68% 32% \/ 46% 64% 36% 54%;/);
  assert.match(trailCss, /translate3d\(var\(--flappy-speed-trail-x,\s*0px\),\s*var\(--flappy-speed-trail-y,\s*0px\),\s*0\)/);
  assert.match(trailCss, /rotate\(var\(--flappy-speed-trail-angle,\s*0deg\)\)/);
  assert.match(trailCss, /background:[\s\S]*radial-gradient[\s\S]*radial-gradient[\s\S]*radial-gradient/);
  assert.doesNotMatch(trailCss, /linear-gradient/);
  assert.match(trailCss, /filter:\s*blur\(2\.25px\) saturate\(1\.08\)/);
  assert.match(trailCss, /will-change:\s*transform,\s*opacity;/);
  assert.doesNotMatch(trailCss, /transition:\s*transform/);
  assert.doesNotMatch(trailCss, /border-radius:\s*999px/);
  assert.match(trailCss, /rotate\(-4deg\)/);
  assert.match(trailCss, /rotate\(6deg\)/);
  assert.match(trailCss, /@keyframes flappy-speed-tail-flow/);
  assert.match(trailCss, /\.flappy-speed-trail-flow::before/);
  assert.match(trailCss, /\.flappy-speed-trail-flow::after/);
  assert.doesNotMatch(trailCss, /box-shadow:\s*0 0 12px/);
});

...
test("advanced lobby slider is controlled by the same selected level as circular clicks", () => {
  ...
  const endlessReviveBankIconCss = cssRule(cssSource, ".advanced-endless-revive-bank-icon");
  ...
  assert.match(screenSource, /import \{ DonateIcon \} from "@\/features\/results\/result-icons";/);
  assert.match(screenSource, /function AdvancedLevelSelectionPanel[\s\S]*reviveCoins,[\s\S]*reviveCoins: number;/);
  assert.match(screenSource, /function AdvancedLobbyContent[\s\S]*reviveCoins,[\s\S]*reviveCoins: number;/);
  assert.match(screenSource, /<AdvancedLevelSelectionPanel[\s\S]*reviveCoins=\{reviveCoins\}/);
  assert.match(screenSource, /<AdvancedLobbyContent[\s\S]*reviveCoins=\{reviveCoins\}/);
  ...
  assert.doesNotMatch(endlessBranchSource, /className=\{?["`][^"`}]*advanced-endless-revive-bank/);
  assert.match(screenSource, /className=\{`advanced-endless-revive-bank \$\{selectedIsEndless \? "visible" : ""\}`\}/);
  assert.match(screenSource, /aria-label=\{`复活币 \$\{reviveCoins\}`\}/);
  assert.match(screenSource, /<DonateIcon \/>/);
  assert.match(screenSource, /<strong>\{reviveCoins\}<\/strong>/);
  ...
  assert.match(cssSource, /\.advanced-endless-revive-bank\s*{/);
  assert.match(cssSource, /\.advanced-endless-revive-bank\s*{[\s\S]*position:\s*absolute;/);
  assert.match(cssSource, /\.advanced-endless-revive-bank\s*{[\s\S]*left:\s*clamp\(2px,\s*0\.7vw,\s*6px\);/);
  assert.match(cssSource, /\.advanced-endless-revive-bank\s*{[\s\S]*top:\s*clamp\(2px,\s*0\.7vw,\s*6px\);/);
  assert.match(cssSource, /\.advanced-endless-revive-bank\s*{[\s\S]*min-height:\s*28px;/);
  assert.match(cssSource, /\.advanced-endless-revive-bank\s*{[\s\S]*padding:\s*0 8px;/);
  assert.match(cssSource, /\.advanced-endless-revive-bank\s*{[\s\S]*border-radius:\s*999px;/);
  assert.match(cssSource, /\.advanced-endless-revive-bank\s*{[\s\S]*background:\s*rgba\(255,\s*253,\s*248,\s*0\.82\);/);
  assert.match(cssSource, /\.advanced-endless-revive-bank\s*{[\s\S]*opacity:\s*0;/);
  assert.match(cssSource, /\.advanced-endless-revive-bank\s*{[\s\S]*transition:/);
  assert.match(cssSource, /\.advanced-endless-revive-bank\.visible\s*{[\s\S]*opacity:\s*1;/);
  assert.match(cssSource, /\.advanced-endless-revive-bank\.visible\s*{[\s\S]*transform:\s*translate3d\(0,\s*0,\s*0\)\s*scale\(1\);/);
  assert.match(cssSource, /\.advanced-endless-revive-bank-icon\s*{/);
  assert.match(endlessReviveBankIconCss, /width:\s*18px;/);
  assert.match(endlessReviveBankIconCss, /height:\s*18px;/);
  assert.match(cssSource, /\.advanced-endless-revive-bank strong\s*{[\s\S]*font-size:\s*15px;/);
